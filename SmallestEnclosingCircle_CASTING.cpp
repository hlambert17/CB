...
static tuple<double, double, double> returnCircleAsTuple(tuple_list &initList) {
	vector<Point> points_casted;
	for (size_t i=0; i<initList.size(); i++) {
		points_casted.push_back(Point{std::get<0>(initList.at(i)), std::get<1>(initList.at(i))});
		}
	Circle c = makeSmallestEnclosingCircle(points_casted);
	return std::make_tuple(c.c.x, c.c.y, c.r);
}
...
PYBIND11_MODULE(SmallestEnclosingCircle_CASTING, m) {
    m.doc() = "Smallest enclosing circle c++ module"; // optional module docstring

    m.def("returnCircleAsTuple", &returnCircleAsTuple, "A function which finds the smallest enclosing circle from a cloud of points");
}
